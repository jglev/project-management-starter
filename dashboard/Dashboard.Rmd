...
for (line in raw_text) {
  ...
  if (!is.na(line_data[1, "start"]) && is.na(line_data[1, "end"])) {
    ...
    line_data[1, "end"] <- NA
    line_data[1, "style"] <- paste0("color: black; background: white; border-bottom: .5em solid ", due_color)
    line_data[1, "start_represents_due"] <- TRUE
  } else {
    # If both start and end are set:
    line_data[1, "style"] <- paste0("color: black; background: white; border-image-source: linear-gradient(to right, ", scheduled_color,", ", due_color, "); border-image-slice: 1; border-bottom: .5em solid; border-top: 1px solid; border-left: 1px solid; border-right: 1px solid;")
  }
...
### Scheduled to start before two weeks from now
...
datatable(
  parsed_data %>% 
    ...
    select(-style, -start_represents_due) %>% 
    rename(
      Status = group,
      Description = content,
      Scheduled = start,
...
### Due in the next week, and Overdue
